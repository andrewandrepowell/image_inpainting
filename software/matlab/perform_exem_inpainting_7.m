function image_array = perform_exem_inpainting_7(image_array,mask_array_0,...
    patch_size,distance_size,skip_factor)

image_size = size(image_array);
[rows,cols] = meshgrid(...
    1:skip_factor:image_size(1),...
    1:skip_factor:image_size(2));
indices_0 = 1:numel(image_array);
...
while any(mask_array_0(:))
    ...
    % Determine isophotes
    iso_row = -imfilter(image_array,grad_mask_col);
    iso_col = imfilter(image_array,grad_mask_row);
    ...
    patch_p_valu = get_patch(image_array,patch_p_location,patch_mask);
    ...
    parfor skip_index=skip_indices
        ...
        if (distance_thres>patch_q_distance_fromp) && (mask_array_1(index) == false)
            
            % Determine important matrices concerning the similar patch q
            patch_q_location = [row col];
            patch_q_valu = get_patch(image_array,patch_q_location,patch_mask);
    ...
    patch_q_valu = get_patch(image_array,mse_location,patch_mask);
    ...
    image_array = set_patch(image_array,patch_q_valu,patch_p_location,patch_replacement_mask);
...
%     close(handle_mse_array); figure(handle_mse_array); imshow(mse_array,[]); title('mse\_array');
    close(handle_image_array);figure(handle_image_array);imshow(image_array);title('image\_array');
